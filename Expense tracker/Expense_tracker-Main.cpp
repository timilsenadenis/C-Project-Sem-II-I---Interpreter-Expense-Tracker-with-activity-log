...
public:
    ...
    static void addExpense(const std::string& username) {
        ...
        allTransactionsFile << username << " " << amount << " " << category << " " << date << "\n";
    ...
    static double calculateTotalExpense(const std::string& filename) {
        ...
        std::string username, category, date;
        double amount, total = 0.0;
        while (expenseFile >> username >> amount >> category >> date) {
            total += amount;
        }
        return total;
    }
    ...
    static void viewExpensesOrPayback(const std::unordered_map<std::string, std::pair<std::string, std::string>>& users, const std::string& username) {
        double totalExpense = calculateTotalExpense("all_transactions.txt");
        double userTotalExpense = calculateUserTotalExpense(username);
        double dividedExpense = totalExpense / users.size();

        double amountDue = dividedExpense - userTotalExpense;
        std::cout << "Divided amount: Rs. " << dividedExpense << "\n";
        std::cout << "Total paid by you: Rs. " << userTotalExpense << "\n";
    ...
    static double calculateUserTotalExpense(const std::string& username) {
        std::ifstream expenseFile("all_transactions.txt");
        std::string uname, category, date;
        double amount, total = 0.0;
        while (expenseFile >> uname >> amount >> category >> date) {
            if (uname == username) {
                total += amount;
            }
        }
